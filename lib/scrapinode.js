...
scrapinode.use = function(route,operation){
    ...
    if(path){
         path = (function(path){
            var normalizePath = path[path.length -1] === '/' ? path : path + '/';
            var explode = normalizePath.split('.');
            return normalizePath[0] === '.' ? __dirname + explode.slice(1,explode.length).join('.') : path;
         })(path);
         try{
